...
/*   Updated: 2022/08/26 09:47:34 by caboudar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	set_enemy_animation(t_mlx *mlx)
{
	...
	while (mlx->game->map[++i])
	{
		...
		while (mlx->game->map[i][++j])
		{
			...
			{
				if (mlx->game->enemy_anim_counter < 5000)
					mlx_put_image_to_window(mlx->mlx, mlx->window,
						mlx->mlx_enemy1_img, (j * 32), (i * 32));
				if (mlx->game->enemy_anim_counter >= 5000
					&& mlx->game->enemy_anim_counter < 10000)
					mlx_put_image_to_window(mlx->mlx, mlx->window,
						mlx->mlx_enemy2_img, (j * 32), (i * 32));
				mlx->game->enemy_anim_counter++;
				if (mlx->game->enemy_anim_counter == 10000)
